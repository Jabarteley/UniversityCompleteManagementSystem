...
router.post('/upload', auth, authorize('academic-staff', 'staff-affairs', 'head-department'), [
  ...
  try {
    ...
    // Find existing semester result or create new one
    ...
    // Update CGPA for all semester results
...
router.get('/student/:regNumber', auth, authorize('student'), async (req, res) => {
  try {
    ...
    const student = await Student.findOne({
      registrationNumber: new RegExp(`^${regNumber.trim()}import express from 'express';
import { body, validationResult } from 'express-validator';
import Student from '../models/Student.js';
import CourseAllocation from '../models/CourseAllocation.js';
import { auth, authorize, AuthRequest } from '../middleware/auth.js';
...
router.get('/student/:regNumber', auth, authorize('student'), async (req, res) => {
  try {
    ...
    const student = await Student.findOne({
      , 'i'), // Trim and case-insensitive search